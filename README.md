...
- [x] 实现 `list_directory`、`find_files`、`read_file`、`grep`、`apply_patch`、`run_command`、`git_status` 和 `git_diff`；
- [x] `apply_patch` 先运行 `git apply --check`，成功后真实修改文件并返回工作区状态；
- [x] `run_command` 返回退出码、stdout、stderr、耗时和超时状态。

M1.3 的工具已经能够被运行时独立调用，但当时尚未接入交互 CLI。

M1.4 将模型决策和内置工具连接成真正的执行循环：

- [x] 交互 CLI 根据当前工作目录创建默认工具注册表，并把 8 个工具 Schema 提供给模型；
...
- [ ] 任务开始时记录 Git 状态，把用户已有的未提交修改作为基线；
- [ ] 在文件修改工具和 Shell 命令执行后重新检查工作区；
- [ ] 记录本次任务实际修改的文件；
- [ ] 使用递增的 `workspace_revision` 标记每一次实际代码变化；
- [ ] 保留用户原有修改，不把它们误认为 Agent 本次产生的修改。

#### M2.2：专用验证工具

- [ ] 新增 `verify` 工具，与普通 `run_command` 明确区分；
- [ ] 复用现有命令执行能力，记录验证命令、退出码、耗时和执行时的 `workspace_revision`；
- [ ] 只有退出码为零的 `verify` 调用才能成为成功验证证据；
- [ ] 验证后如果代码再次变化，旧验证自动失效。

#### M2.3：完成检查

- [ ] 模型准备结束代码任务时，检查是否真的产生了修改；
- [ ] 检查当前工作区版本是否有成功的验证证据；
- [ ] 测试失败、没有验证或验证已经过期时拒绝结束；
- [ ] 修改禁止路径或允许范围外的文件时拒绝结束；
- [ ] 执行 `git diff --check`，存在确定性的 Patch 错误时拒绝结束；
- [ ] 拒绝结束时把具体原因返回模型，让 Agent Loop 继续工作；
- [ ] 连续三次仍不能满足条件时，以 `blocked` 结束并说明原因，避免无限循环；
- [ ] 最终结果展示修改文件、验证命令、退出码和耗时。

### 实现要求

- 工作区变化、验证结果和完成检查继续使用现有事件与 JSONL 轨迹记录；
- system prompt 告诉模型修改代码后使用 `verify`，但是否完成由运行时代码判断；
- 为工作区变化、验证成功与失败、验证过期、越界修改和完成检查编写自动化测试；
- 在干净副本中运行三个 Fixture，Agent 不能读取或修改隐藏测试，完成后由评测器独立运行隐藏测试；
- 整个流程直接运行在本机，不要求 Docker。
...
- ForgeCode 能发现模型和 Shell 命令修改了哪些文件；
- 测试失败、没有验证或验证已过期时，系统能够拒绝结束；
- 修改禁止路径或允许范围外文件时，系统能够拒绝结束；
- 用户原有的未提交修改不会被覆盖或误认为本次 Agent 修改；
- 三个 Fixture 均在干净副本中完成公开测试和隐藏测试验收；
- 最终结果能够列出修改文件和验证依据，并留下可回放的 JSONL 轨迹。
