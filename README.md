...
`.env` 已被 Git 忽略，仓库只提交不含真实凭据的 `.env.example`。系统环境变量优先于 `.env` 中的同名配置。`ANTHROPIC_API_KEY` 和 `MODEL_ID` 必填；`ANTHROPIC_BASE_URL` 可以省略，默认使用 `https://api.anthropic.com`。`MODEL_MAX_TOKENS` 可选，默认 `8192`，允许范围为 `1024～32768`。`forge config` 显示 Model ID、Base URL、最大输出 Token 和密钥配置状态，但不会回显 API Key。交互模式的每轮消息都会发起真实 API 请求，可能产生 Provider 费用。
...
- [x] 协议错误和输出截断最多自动恢复两次，失败响应中的工具一律不执行；
