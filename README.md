...
- **可恢复而非重来**：采用 Codex 风格的追加式 Session Rollout；对话通过 resume/fork 恢复和分叉，代码版本与回滚仍交给 Git。
...
- 每个稳定生命周期事件和消息增量实时写入 `.forge/sessions/*.rollout.jsonl`，异常恢复不会重复执行已有成功结果的工具；
...
非 Git 工作区在首个 Agent turn 建立基线时会自动初始化私有 Git 元数据：项目根目录只生成一个 `.git` 指针文件，真实 gitdir 保存在受保护的 `.forge/git/` 中，且 `.forge/` 通过仓库本地 exclude 排除。已有项目文件会进入基线，不会被误算为 Agent 本轮修改；之后新建或改写的文件可正常进入 Diff、验证和完成检查。
...
### 当前状态：权限预设与交互审批已实现，操作系统沙箱与回滚待实现

在交互终端输入 `/permissions` 可打开与 `/` 命令补全相同风格的内联权限菜单，使用方向键选择、Enter 确认、Esc 取消；选择后立即对当前会话生效：

- `Read Only`：只允许只读工具，阻止写入和进程操作；
- `Ask for approval`：每次写入或进程操作前显示内联确认菜单；
- `Approve for me`：自动批准工作区编辑和低风险本地命令，高风险或外部操作仍弹窗；
- `Full Access`：不弹出审批窗口，但工作区边界和工具自身的安全检查仍然有效。

审批菜单支持“仅允许这次”“本会话允许相似操作”和“拒绝”。权限查看与切换统一使用 `/permissions`。当前权限层是 ForgeCode 的工具执行策略，并不等同于操作系统级沙箱；不可信仓库、高风险命令和完整回滚能力仍需完成本里程碑的剩余项目。
...
- [x] `Read Only`：只读和分析，不修改文件；
- [x] `Ask for approval`：代码编辑和进程操作需要确认；
- [x] `Approve for me`：低风险行为自动执行，高风险行为仍需确认；
- [x] `Full Access`：工具无需逐次确认；
- [ ] 支持 `allow`、`ask` 和 `deny`，并定义规则优先级；
- [ ] 支持用户级、项目级和会话级规则；
- [x] 支持“允许一次”“本会话允许”和“拒绝”；
- [ ] 支持“当前项目允许”和“拒绝并告知 Agent 原因”。
...
当前有意保持简单：不使用向量数据库、Embedding 或云端记忆。项目规则和记忆是模型上下文，不代替 M3 的强制权限控制。
...
在自然语言消息的任意空白位置输入 `@`，终端会列出当前 workspace 中可引用的文件；继续输入文件名或路径片段可进行大小写不敏感的前缀、包含和模糊匹配。选中后会插入 `@相对路径`。每次显示新输入框前都会刷新文件索引，因此 Agent 上一轮新建的文件可以立即被选择；`.git/`、`.forge/`、私有 `.env*`、依赖和构建缓存不会出现在候选中。

回答生成或工具执行期间按 `Esc` 会中断当前这一轮并返回输入提示，不会退出 ForgeCode。已经完成的文件修改和工具副作用不会自动回滚；正在运行的前台子进程会被终止。`Ctrl+C` 仍用于退出整个交互会话。
...
- Task Subagent 能完成仓库调查、编辑和验证；
- 子 Agent 不能递归创建子 Agent，且不会绕过主 Agent 的权限策略；
