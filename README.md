...
首个 Model Provider 确定为 Anthropic。ForgeCode 直接复用官方 Python SDK 的 MessageParam 和 ToolParam 类型，ModelClient 将 AsyncAnthropic.messages.stream 转换为 Provider 无关的文本 delta 与 usage 更新事件。
...
M1.3 的工具已经能够被运行时独立调用，但 CLI 尚未把 Schema 交给模型，也没有执行 Tool Call 后把 `tool_result` 反馈给模型。这个多步执行循环属于 M1.4。命令审批、敏感路径治理和更强的本机安全限制仍属于 M3；当前工具层不是安全沙箱。
